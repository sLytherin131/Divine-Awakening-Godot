...
@export var enemy_dragon: Resource = null
...
var current_dragon_health = 0

func _ready():
	# Mulai musik otomatis
	play_music(music_path, audio_bus, -10)
	set_health($PlayerPanel/PlayerData/Player/ProgressBar_mc1, StateProgressBar.current_health, StateProgressBar.max_health)
	set_health($VBoxContainer/ProgressBar_dg, enemy_dragon.health, enemy_dragon.health)
	$BlankPanel.hide()
	$SettingPanel.hide()
	$RetryPanel.hide()
	$VBoxContainer/Enemy/Orc.texture = enemy_dragon.texture
	current_player_health = StateProgressBar.current_health
	current_dragon_health = enemy_dragon.health
...
func _on_attack_pressed() -> void:
	...
	var attack_damage = randf_range(10, 35)  # Rentang damage antara 500 hingga 1500
	# Tentukan pesan berdasarkan damage
	if attack_damage < 15:
		display_text("Sialan, hanya goresan saja!")
	elif attack_damage <= 25:
		display_text("Ini baru yang saya inginkan!")
	else:
		display_text("A devastating blow!")
	await get_tree().create_timer(1).timeout
	play_effect_sound("res://music/bgm/punchsfx.mp3")
	await get_tree().create_timer(0.3).timeout
	# Kurangi health musuh
	current_dragon_health = max(0, current_dragon_health - attack_damage)
	set_health($VBoxContainer/ProgressBar_dg, current_dragon_health, enemy_dragon.health)
	$AnimationPlayer.play("dragon_anim")
	
	await $AnimationPlayer.animation_finished
	if current_dragon_health <= 0:
		display_text("Ravagius Orc sudah di kalahkan!")
		stop_music()  # Hentikan musik setelah display_text ditampilkan
		await get_tree().create_timer(1).timeout
		var _dialogic = Dialogic.start("res://timeline/timeline1_after_battle_dragon.dtl")
	else:
...
func enemy_attack() -> void:
	...
	var enemy_damage = randf_range(5, 25)
	# Kurangi health pemain
	current_player_health = max(0, current_player_health - enemy_damage)
	set_health($PlayerPanel/PlayerData/Player/ProgressBar_mc1, current_player_health, StateProgressBar.max_health)
	if enemy_damage < 10:
		display_text("Serangan Orc itu lemah!")
	elif enemy_damage <= 20:
		display_text("Serangan Orc cukup menyakitkan!")
	else:
		display_text("Serangan Orc sangat mematikan!")
	await get_tree().create_timer(2).timeout
	if current_player_health <= 0:
		display_text("Kamu telah dikalahkan oleh Ravagius Orc!")
